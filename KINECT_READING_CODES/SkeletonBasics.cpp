...
int CSkeletonBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	fileOutput = std::ofstream("saida.txt");
	fileOutput << "SENSOR\t\tX\t\tY\t\tZ\n";
...
{      
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
		...
		//Salva as posicoes de cada junta no arquivo de texto
		fileOutput <<skel.SkeletonPositions[i].x << "," <<skel.SkeletonPositions[i].y << "," <<skel.SkeletonPositions[i].z ;
